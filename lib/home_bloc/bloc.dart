...
import 'package:baru_nih/home_bloc/bloc_event.dart';
...
class HomeBloc extends Bloc<HomeBlocEvent, HomeBlocState>{
  ...
  Stream<HomeBlocState> mapEventToState(currentState, event) async* {
    if (event is HomeGetData && !_hasReachedMax(currentState)) {
      try {
        if (currentState is HomeUninitialized) {
          final dataList = await _fetchData(event.bulanTahun);
          //=========throw ke block state=======
          //=========Return nilai home data reload, but ga stop fungsinya. Fungsi tetap jalan=======
